...
class SmsActivation:
	...
	def cancel(self):
		set_status = activationpw.SetStatus(
			id=self.__id,
			status=activationpw.SmsTypes.Status.Cancel
		)
		if self.wrapper:
	...
	def mark_as_used(self):
		set_status = activationpw.SetStatus(
			id=self.__id,
			status=activationpw.SmsTypes.Status.AlreadyUsed
		)
		if self.wrapper:
			return set_status.request(self.wrapper)
		return set_status
	...
	def was_sent(self):
		set_status = activationpw.SetStatus(
			id=self.__id,
			status=activationpw.SmsTypes.Status.SmsSent
		)
		if self.wrapper:
			return set_status.request(self.wrapper)
		return set_status
	...
	def wait_code(self, timeout=1200, callback=None, not_end=False, *args, **kwargs):
		"""
		:param wrapper: obj for work with activationpw
		...
		while True:
			...
			if counter >= timeout:
				raise ('Timeout error')
			response = activationpw.GetStatus(id=self.id).request(self.wrapper)
			if response['code'] and not not_end and response['code'] != self.last_code:
				self.__last_code = response['code']
				activationpw.SetStatus(
					id=self.id,
					status=activationpw.SmsTypes.Status.End
				...
				activationpw.SetStatus(
					id=self.id,
					status=activationpw.SmsTypes.Status.OneMoreCode
				)
				break
		if callback:
